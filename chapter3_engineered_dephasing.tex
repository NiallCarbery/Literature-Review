...
\qquad We now demonstrate the aforementioned dephasing effects by applying it to the density matrix formalism of a simplified 'toy' Landau Zener model. The meter coupling induces dephasing, this is shown through a single qubit initialized in the superposition state $|+\rangle = \tfrac{1}{\sqrt{2}}(|0\rangle + |1\rangle)$. Then selecting the meter observable as $X_M=x_0\sigma_z$ and assume negligible intrinsic meter dynamics ($H_M=0$), while keeping the system Hamiltonian $H_S(t)$ general. Here, $x_0$ represents the coupling strength of the QND-like interaction. The resulting Hamiltonian is given by
%%
\begin{equation}
