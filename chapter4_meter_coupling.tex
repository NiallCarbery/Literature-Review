...
\qquad We understand the We begin by writing the full system--meter Hamiltonian from Chapter~\ref{Ch3:SpeedingUpQuantumAnnealing} in its most general form,
%%
\begin{equation}\label{eq:total_hamiltonian_nc}
    H(t) = H_S(t) \otimes \mathds{1}_M + x_0\, H_S(t) \otimes X_M + \mathds{1}_S \otimes H_M,
\end{equation}
%%
where $H_S(t)$ is the time-dependent system Hamiltonian, $X_M$ is the meter observable that couples to the system, $x_0$ is the coupling strength, and $H_M$ governs the free meter dynamics. Throughout this chapter we specialise to a qubit meter with $X_M = \sigma_z, H_M = \omega\,\sigma_x$ so that $[H_M, X_M] = [\omega\sigma_x, \sigma_z] = -2i\omega\sigma_y \neq 0$ for any $\omega \neq 0$. The parameter $\omega$ therefore quantifies the departure from the ideal commuting regime. The total Hamiltonian becomes
%%
...
\color{red} Show this \color{black}
...
\end{equation}
%%
with $\Omega_n(t) = \sqrt{(x_0 E_n(t))^2 + \omega^2}$. The first term is the static component, set by the product $\cos 2\theta_n(0)\,\cos 2\theta_n(t)$. Crucially, it depends on both the initial energy $E_n(0)$ and the instantaneous energy $E_n(t)$. The second term is an oscillatory component at twice the Rabi frequency. Its amplitude $\omega^2/(\Omega_n(0)\Omega_n(t))$ is largest when $\omega$ dominates both the initial and instantaneous coupling energies, and vanishes in the commuting limit $\omega \to 0$.

%%==========================================================================
\section{Time-Averaged Rescaling and Speedup Bound}\label{sec:time_averaged_rescaling}
%%==========================================================================

\qquad To extract the net effect of the non-commuting dynamics on the speedup, we time-average the meter polarisation in Eq.~\eqref{eq:sigma_z_final}. Over timescales much longer than the Rabi period $2\pi/\Omega_n$, the oscillatory cosine term averages to zero, leaving only the static component
%%
...
\end{equation}
%%
For the ground state ($n=0$), the effective rescaling factor becomes $1 + x_0\,\overline{\langle\sigma_z\rangle}_0$ rather than the ideal $(1+x_0)$. Since $|\overline{\langle\sigma_z\rangle}_0| \leq 1$ for all $\omega > 0$, with equality only at $\omega = 0$, the time-averaged rescaling is strictly reduced by non-commuting meter dynamics. This establishes the following bound on the achievable speedup: \color{red} Show this \color{black}
%%
\begin{equation}\label{eq:speedup_bound}
    \text{Speedup} \leq 1 + x_0\,\overline{\langle\sigma_z\rangle}_0 \leq 1 + x_0 = 1 + m_{\max},
\end{equation}
%%
with equality if and only if $\omega = 0$. The bound $1 + m_{\max}$ derived in Chapter~\ref{Ch3:SpeedingUpQuantumAnnealing} for the commuting case is therefore the global maximum, and any non-commuting meter dynamics strictly degrades the speedup. The degree of degradation is controlled by the ratio $\omega/(x_0 E_n)$: when $\omega \ll x_0 E_n$, the mixing angle $\theta_n \approx 0$ and the polarisation remains close to unity; when $\omega \gg x_0 E_n$, the polarisation falls toward zero and the meter coupling becomes ineffective.
...
\qquad When the meter has non-commuting dynamics $H_M = \omega\sigma_x$, the relevant Rabi frequency for sector~$n$ is, from Eq.~\eqref{eq:sigma_z_final},
%%
\begin{equation}\label{eq:effective_rabi}
    \Omega_n(t) = \sqrt{\bigl(x_0\,E_n(t)\bigr)^2 + \omega^2},
\end{equation}
%%
where $E_n(t)$ is the instantaneous system energy eigenvalue. From Eq.~\eqref{eq:sigma_z_final}, the oscillatory part of $\langle\sigma_z\rangle_n$ defines the interference factor
%%
