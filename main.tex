...
Fig.~\ref{fig:interference_analysis} displays the results of this analysis. The fidelity map exhibits characteristic ripple patterns at small $\omega$ and short $T$, which align precisely with the zero-crossings of the analytical interference factor $C(T, \omega) = 0$ (white dashed contours). These contours occur when the accumulated phase satisfies $2x_0 \int_0^T \Omega_{\text{eff}}(s)\,ds = (2n+1)\pi/2$ for integer $n$, corresponding to complete destructive interference between the two evolution branches.
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.32\textwidth]{NonCommutation/WNonFidDiff.png}
        \includegraphics[width=0.32\textwidth]{NonCommutation/InterferenceFactor.png}
        \includegraphics[width=0.32\textwidth]{NonCommutation/WNonCommuting.png}
        \caption{Analysis of interference effects in the non-commuting meter regime. \textbf{Left:} Fidelity difference $\Delta F = F(T, \omega) - F(T, 0)$ showing ripple modulations at small $\omega$. \textbf{Center:} Analytical interference factor $C(T, \omega) = \cos(2x_0\int\Omega_{\text{eff}}\,ds)$ with zero-crossing contours (black dashed). \textbf{Right:} Fidelity map with $C=0$ contours overlaid (white dashed), demonstrating the correspondence between numerical fidelity ripples and analytical predictions.}
