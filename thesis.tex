...
% Ensure captions can be centered across the full text width
\usepackage{caption}
...
\section{Speeding up Quantum Annealing Using Engineered Dephasing}
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.45\textwidth}{\centering (a)}
        \hspace{0.02\textwidth}
        \parbox{0.45\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.45\textwidth]{Dephasing/Dephasing1a.png}\label{fig:landau_zener_results_a}
        \hspace{0.02\textwidth}%
        \includegraphics[width=0.45\textwidth]{Dephasing/Dephasing1b.png}\label{fig:landau_zener_results_b}
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.4\textwidth}{\centering (a)}
        \hspace{0.02\textwidth}
        \parbox{0.4\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.4\textwidth]{Dephasing/Dephasing2a.png}\label{fig:fidelity_contour_a}
        \hspace{0.02\textwidth}
        \includegraphics[width=0.4\textwidth]{Dephasing/Dephasing2b.png}\label{fig:fidelity_contour_b}
    ...
    \begin{figure}[ht]
        ...
        \hspace{0.02\textwidth}
        \parbox{0.4\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.4\textwidth]{Dephasing/Dephasing4a.png}\label{fig:fidelity_diff_a}
        \hspace{0.02\textwidth}
        \includegraphics[width=0.4\textwidth]{Dephasing/Dephasing4b.png}\label{fig:fidelity_diff_b}
    ...
    \begin{figure}[htbp]
        ...
        \hspace{0.02\textwidth}
        \parbox{0.45\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.45\textwidth]{NonCommutation/LZRescale.png}\label{fig:NonCommutationRescaling_a}
        \hspace{0.02\textwidth}
        \includegraphics[width=0.45\textwidth]{NonCommutation/QARescale.png}\label{fig:NonCommutationRescaling_b}
        \caption{Impact of non-commuting meter dynamics on protocol performance. The plots show the effective energy rescaling factor $1+x_0\langle\sigma_z\rangle(t)$ relative to the ideal QND case as a function of duration $T$ and $\omega$. (a) shows the Landau-Zener qubit, and (b) shows the 3-qubit Ising outcome. The universally negative values indicate that introducing a non-commuting term $[X_M, H_M] \neq 0$ strictly reduces the effectiveness of the speedup.}
...
Fig.~\ref{fig:interference_analysis} displays the results of this analysis. The fidelity map exhibits characteristic ripple patterns at small $\omega$ and short $T$, which align precisely with the zero-crossings of the analytical interference factor $C(T, \omega) = 0$ (white dashed contours). These contours occur when the accumulated phase satisfies $2x_0 \int_0^T \Omega_{\text{eff}}(s)\,ds = (2n+1)\pi/2$ for integer $n$, corresponding to complete destructive interference between the two evolution branches.
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.32\textwidth}{\centering (a)}
        \parbox{0.32\textwidth}{\centering (b)}
        \parbox{0.32\textwidth}{\centering (c)}\\[0.5ex]
        \includegraphics[width=0.32\textwidth]{NonCommutation/WNonFidDiff.png}\label{fig:interference_analysis_a}
        \includegraphics[width=0.32\textwidth]{NonCommutation/InterferenceFactor.png}\label{fig:interference_analysis_b}
        \includegraphics[width=0.32\textwidth]{NonCommutation/WNonCommuting.png}\label{fig:interference_analysis_c}
        \caption{Analysis of interference effects in the non-commuting meter regime. (a) Fidelity difference $\Delta F = F(T, \omega) - F(T, 0)$ showing ripple modulations at small $\omega$. (b) Analytical interference factor $C(T, \omega) = \cos(2x_0\int\Omega_{\text{eff}}\,ds)$ with zero-crossing contours (black dashed). (c) Fidelity map with $C=0$ contours overlaid (white dashed), demonstrating the correspondence between numerical fidelity ripples and analytical predictions.}
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.34\textwidth}{\centering (a)}
        \hfill
        \parbox{0.63\textwidth}{\centering (b)}\\[0.5ex]
    ...
    \begin{figure}[htbp]
        ...
        \hspace{0.02\textwidth}
        \parbox{0.45\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.45\textwidth]{OptimalRamp/LZOptimalRamp.png}\label{fig:optimal_ramp_a}
        \hspace{0.02\textwidth}%
        \includegraphics[width=0.45\textwidth]{OptimalRamp/QAOptimalRamp.png}\label{fig:optimal_ramp_b}
        \caption{Models with energy levels and optimal local schedule compared to linear schedule. The optimal schedule slows down significantly near the minimum gap region, allowing the system to remain adiabatic through the critical point, while speeding up in regions with larger gaps. (a) Landau Zener Model (b) QA Model with 3 Qubits}
    \end{figure}
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.35\textwidth}{\centering (a)}
        \hspace{0.02\textwidth}
        \parbox{0.35\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.35\textwidth]{OptimalRamp/LZOptimalDiff.png}
        \hspace{0.02\textwidth}%
        \includegraphics[width=0.35\textwidth]{OptimalRamp/QAOptimalDiff.png}
        \caption{Fidelity difference between optimal local adiabatic and linear schedules. (a) Landau Zener Model (b) QA Model with 3 Qubits averaged over 100 instances with schedule optimized for each instance.}
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.3\textwidth}{\centering (a)}
        \parbox{0.3\textwidth}{\centering (b)}
        \parbox{0.3\textwidth}{\centering (c)}\\[0.5ex]
        \includegraphics[width=0.3\textwidth]{figures/GapStats/3_GapProfiles.png}\label{fig:gap_profiles_a}
        \includegraphics[width=0.3\textwidth]{figures/GapStats/4_GapProfiles.png}\label{fig:gap_profiles_b}
        \includegraphics[width=0.3\textwidth]{figures/GapStats/5_GapProfiles.png}\label{fig:gap_profiles_c}
        \caption{Characteristic Energy Gaps for (a) 3 qubit, (b) 4 qubit, and (c) 5 qubit random Ising instances with $J_{ij}\in[0,1]$. Average and standard deviation are indicated for 100{,}000 iterations of random samples.}
        \label{fig:gap_profiles}
    \end{figure}

    \qquad We first make a crude analysis and apply the average schedule derived from the average gap profile to all instances. We compare this to the performance of the individual optimal schedules averaged over many instances, specifically for Ising instances with different random parameters. We list some statistical properties of the gaps in the Appendix \ref{sec:appendix_gap_statistics}.

    \qquad We first compare the performance of the average schedule to the linear schedule. From there it is critical to examine how much performance is lost to the averaging process by comparing to the individual optimal schedules.

    \begin{figure}[htbp]
        \centering
        \parbox{0.3\textwidth}{\centering (a)}
        \parbox{0.3\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.3\textwidth]{figures/OptimalRamp/3-QAAverageRampVsLinear.png}\label{fig:average_schedule_a}
        \includegraphics[width=0.3\textwidth]{figures/OptimalRamp/3-QAAverageVsOptimal.png}\label{fig:average_schedule_b}
        \caption{Averaged optimal local adiabatic schedule performance for 3 qubit random Ising instances. (a) Comparison of average optimal schedule to linear schedule. (b) Comparison of average optimal schedule to individual optimal schedules averaged over many instances.}
...
\begin{refsection}
    ...
    \begin{figure}[htbp]
        ...
        \parbox{0.32\textwidth}{\centering (b)}
        \parbox{0.32\textwidth}{\centering (c)}\\[0.5ex]
        \includegraphics[width=0.32\textwidth]{LargerIsings/2-2b.png}\label{fig:larger_isings_2b_a}
        \includegraphics[width=0.32\textwidth]{LargerIsings/3-2b.png}\label{fig:larger_isings_2b_b}
        \includegraphics[width=0.32\textwidth]{LargerIsings/4-2b.png}\label{fig:larger_isings_2b_c}
        \caption{Larger Ising systems variant of Fig. \ref{fig:fidelity_contour}. (a) 2-qubit, (b) 3-qubit, (c) 4-qubit systems.}
    ...
    \begin{figure}[htbp]
        ...
        \caption{Larger Ising systems variant of Fig. \ref{fig:fidelity_contour}. 5-qubit system.}
    ...
    \begin{figure}[htbp]
        ...
        \parbox{0.32\textwidth}{\centering (b)}
        \parbox{0.32\textwidth}{\centering (c)}\\[0.5ex]
        \includegraphics[width=0.32\textwidth]{LargerIsings/3-4b.png}\label{fig:larger_isings_4b_a}
        \includegraphics[width=0.32\textwidth]{LargerIsings/4-4b.png}\label{fig:larger_isings_4b_b}
        \includegraphics[width=0.32\textwidth]{LargerIsings/5-4b.png}\label{fig:larger_isings_4b_c}
        \caption{Larger Ising systems variant of Fig. \ref{fig:fidelity_diff}. (a) 3-qubit, (b) 4-qubit, (c) 5-qubit systems.}
    ...
    \begin{figure}[htbp]
        \centering
        \parbox{0.32\textwidth}{\centering (a)}
        \parbox{0.32\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.32\textwidth]{LargerIsings/6-4b.png}\label{fig:larger_isings_6_7_a}
        \includegraphics[width=0.32\textwidth]{LargerIsings/7-4b.png}\label{fig:larger_isings_6_7_b}
        \caption{Larger Ising systems continued. (a) 6-qubit, (b) 7-qubit systems.}
    ...
    \begin{figure}[htbp]
        ...
        \parbox{0.45\textwidth}{\centering (a)}
        \parbox{0.45\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.45\textwidth]{LargerIsings/FidelityIncrease.png}\label{fig:larger_systems_fidelity_a}
        \includegraphics[width=0.45\textwidth]{LargerIsings/FidelityIncreaseMesh.png}\label{fig:larger_systems_fidelity_b}
        \caption{\textbf{Possible Graphing for Larger Systems need more Data points}. (a) Fidelity increase, (b) Fidelity increase mesh.}
    ...
    \qquad Below are average annealing schedules for different random Ising instances with $N=3,4,5$ qubits, each averaged over 100,000 samples. These plots illustrate the typical shapes of the annealing functions used in the simulations. We compare boht those agasint the most optimal schedule for each instance and the linear schedule.
    \begin{figure}[htbp]
        \centering
        \parbox{0.48\textwidth}{\centering (a)\\[0.5ex]\includegraphics[width=0.4\textwidth]{OptimalRamp/3-QAAverageRampVsLinear.png}\label{fig:opt_ramp_3_a}}
        \parbox{0.48\textwidth}{\centering (b)\\[0.5ex]\includegraphics[width=0.4\textwidth]{OptimalRamp/3-QAAverageVsOptimal.png}\label{fig:opt_ramp_3_b}}\\[1ex]
        \parbox{0.48\textwidth}{\centering (c)\\[0.5ex]\includegraphics[width=0.4\textwidth]{OptimalRamp/4-QAAverageVsLinear.png}\label{fig:opt_ramp_4_a}}
        \parbox{0.48\textwidth}{\centering (d)\\[0.5ex]\includegraphics[width=0.4\textwidth]{OptimalRamp/4-QAAverageVsOptimal.png}\label{fig:opt_ramp_4_b}}\\[1ex]
        \parbox{0.48\textwidth}{\centering (e)\\[0.5ex]\includegraphics[width=0.4\textwidth]{OptimalRamp/5-QAAverageVsLinear.png}\label{fig:opt_ramp_5_a}}
        \parbox{0.48\textwidth}{\centering (f)\\[0.5ex]\includegraphics[width=0.4\textwidth]{OptimalRamp/5-QAAverageVsOptimal.png}\label{fig:opt_ramp_5_b}}
        \caption{Average annealing schedules compared to the linear schedule and the instance-optimal schedule for $N=3$--$5$. Images on the left repersent the improvement of the average schedule compared to a linear schedule, while images on the right show the degradation compared to the instance-optimal schedule: (a,b) $N=3$, (c,d) $N=4$, (e,f) $N=5$.}
    ...
    As part of investigating regimes of meter impact we investigate different regimes of coupling strengths. Throughout this thesis we have remained in the ferromagnetic regime i.e. $J_{ij}\in[0,1]$. Here we present results for both antiferromagnetic couplings $J_{ij}\in[-1,0]$ and mixed couplings $J_{ij}\in[-1,1]$. These coupling strengths are allowed in QUBO formualted annealing problems and thus there effect is included. The results are presented in the following figures, which are variants of Figs. \ref{fig:fidelity_contour} and \ref{fig:fidelity_diff} from the main text 
    ...
    \begin{figure}[htbp]
        ...
        \hspace{0.02\textwidth}
        \parbox{0.45\textwidth}{\centering (b)}\\[0.5ex]
        \includegraphics[width=0.4\textwidth]{figures/AntiFerromagnetic/Peak0mesh2b.png}\label{fig:antif_peak0fig}
        \hspace{0.02\textwidth}%
        \includegraphics[width=0.4\textwidth]{figures/AntiFerromagnetic/Peak0Fig4b.png}\label{fig:antif_peak0mesh}
        \\
        \vspace{1ex}
        \parbox{0.4\textwidth}{\centering (c)}
        \hspace{0.02\textwidth}
        \parbox{0.3\textwidth}{\centering (d)}\\[0.5ex]
        \includegraphics[width=0.4\textwidth]{figures/AntiFerromagnetic/AntiF-mesh2b.png}\label{fig:antif_fig4b}
        \hspace{0.02\textwidth}%
        \includegraphics[width=0.4\textwidth]{figures/AntiFerromagnetic/AntiF-Fig4b.png}\label{fig:antif_mesh2b}
        \caption{Antiferromagnetic / mixed-coupling regime results. (a-b) Represents the mixed coupling regime $J_{ij}\in[-1,1]$, (c-d) Represents the antiferromagnetic coupling regime $J_{ij}\in[-1,0]$. (a,c) Displayed increase in Fidelity with interaction strength as in Fig \ref{fig:fidelity_contour} (b,d) Fidelity Reduction relative to intrinsic meter dynamics as in Fig \ref{fig:fidelity_diff}.}
        \label{fig:antiferromagnetic_results}
    \end{figure}


    \qquad These regimes display that the ferromagnetic regime repersents the worst case results for meter coupling and non commuting effects. Both the antiferromagnetic and mixed coupling regimes display significantly higher fidelities across the parameter space. Thus we the ferromagnetic regime is the most challenging for meter coupling, and thus the results presented in the main text repersent a lower bound on performance across all coupling regimes.
    ...
    \subsubsection{Beyond the Two Unitary Branches: Full Quantum Dynamics}
    
    \qquad In the commuting case $[X_M, H_M] = 0$, the meter remains in an eigenstate of $X_M$ throughout evolution, leading to a clean decomposition into two unitary branches with rescaled Hamiltonians $(1 \pm x_0)H_S(t)$. However, when $[X_M, H_M] \neq 0$, this picture breaks down fundamentally. The meter state evolves dynamically, entangling with the system in a time-dependent manner that cannot be captured by a simple two-branch description.
    
    \qquad Consider the full system-meter Hamiltonian with $H_M = \omega \sigma_x$ and coupling $X_M = \sigma_z$:
    \begin{equation}
        H_{\text{tot}}(t) = H_S(t) \otimes \mathds{1}_M + x_0 H_S(t) \otimes \sigma_z + \mathds{1}_S \otimes \omega \sigma_x.
    \end{equation}
    
    The key observation is that this Hamiltonian does \emph{not} decompose into independent sectors. Unlike the commuting case where $H_{\text{tot}}$ is block-diagonal in the meter basis, the transverse field $\omega \sigma_x$ couples the meter branches at every instant, creating a continuously evolving entangled state.
    
    \paragraph{Rotating Frame Analysis}
    
    To understand the dynamics, we work in the instantaneous eigenbasis of $H_S(t) = \sum_n E_n(t) |n(t)\rangle \langle n(t)|$. For a fixed system eigenstate $|n(t)\rangle$, the meter experiences an effective $2 \times 2$ Hamiltonian:
    \begin{equation}
        H_{\text{meter}}^{(n)}(t) = E_n(t) (1 + x_0 \sigma_z) + \omega \sigma_x = \begin{pmatrix} E_n(t)(1+x_0) & \omega \\ \omega & E_n(t)(1-x_0) \end{pmatrix}.
    \end{equation}
    
    This is \emph{not} diagonal—the off-diagonal elements $\omega$ indicate that the meter's $|\uparrow\rangle$ and $|\downarrow\rangle$ states (eigenstates of $\sigma_z$) are continuously mixing. Diagonalizing yields instantaneous eigenvalues:
    \begin{equation}
    ...
    \begin{equation}
        |\psi_{n,\pm}(t)\rangle = \cos\theta_n(t) |\uparrow\rangle \pm \sin\theta_n(t) |\downarrow\rangle,
    \end{equation}
    ...
    This time-dependent mixing angle is the mathematical signature that the system cannot be described by two static branches. Instead, the meter state precesses between the $|\uparrow\rangle$ and $|\downarrow\rangle$ states with a frequency governed by $\Omega_n(t)$.
    
    \paragraph{Kraus Operator Representation}
    
    To find the reduced system dynamics after tracing out the meter, we must account for the full time-evolution of the coupled system-meter state. Starting with the meter in $|\uparrow\rangle$ (eigenstate of $\sigma_z$ with eigenvalue $+1$), the joint state evolves as:
    \begin{equation}
        |\Psi(t)\rangle = \sum_n c_n(t) |n(t)\rangle \otimes |\chi_n(t)\rangle,
    \end{equation}
    where $|\chi_n(t)\rangle$ is the meter state conditioned on the system being in eigenstate $|n(t)\rangle$. Crucially, $|\chi_n(t)\rangle$ is \emph{not} constant—it undergoes Rabi oscillations.
    
    The reduced density matrix after tracing out the meter becomes:
    \begin{equation}
    ...
    This result demonstrates three crucial differences from the two-branch picture:
    
    \begin{enumerate}
        \item \textbf{Time-dependent coupling:} The effective rescaling factor $1 + x_0\langle\sigma_z\rangle_n(t)$ oscillates in time, unlike the constant rescaling $(1 \pm x_0)$ in the commuting case.
        
        \item \textbf{Energy-dependent dephasing:} Different energy levels experience different Rabi frequencies $\Omega_n(t)$, causing the meter to decorrelate between system eigenstates at different rates.
        
        \item \textbf{Loss of interference:} The cosine interference pattern from the two-branch description is destroyed because the meter is no longer in a coherent superposition of $|\uparrow\rangle$ and $|\downarrow\rangle$—it has been driven into a mixed state by the transverse field.
    \end{enumerate}
    
    The first term $\frac{(x_0 E_n(t))^2}{\Omega_n(t)^2}$ represents the time-averaged (dc) component of the polarization, while the second term captures the coherent oscillations. Averaging over the rapid phase cycles (valid when $\Omega_n(t)$ is large compared to the system evolution timescale) removes the cosine term, yielding:
    \begin{equation}
    ...
    \paragraph{Breakdown at Avoided Crossings}
    
    The expression reveals a critical vulnerability near avoided crossings where $E_n(t) \to E_{\min} \approx 0$. At these points:
    \begin{equation}
        \overline{\langle\sigma_z\rangle} \to \frac{(x_0 E_{\min})^2}{(x_0 E_{\min})^2 + \omega^2} \ll 1 \quad \text{when } \omega \gg x_0 E_{\min}.
    \end{equation}
    
    The effective coupling vanishes precisely where the speedup is most needed. For the Landau-Zener model with minimum gap $E_{\min} = g/2$, the minimum effective rescaling factor is:
    \begin{equation}
        r_{\min}^{\text{eff}} = 1 + x_0 \frac{(x_0 g/2)^2}{(x_0 g/2)^2 + \omega^2}.
    \end{equation}
    
    \paragraph{Comparison with Two-Branch Model}
    
    In the commuting case, the reduced density matrix is exactly:
    \begin{equation}
        \rho_S^{\text{comm}}(t) = \tfrac{1}{2}U_+(t)\rho_S(0)U_+^\dagger(t) + \tfrac{1}{2}U_-(t)\rho_S(0)U_-^\dagger(t),
    \end{equation}
    with $U_\pm(t) = \mathcal{T}\exp\left[-i\int_0^t (1\pm x_0)H_S(s)ds\right]$.
    
    In the non-commuting case, we must generalize to a continuous Kraus decomposition:
    \begin{equation}
        \rho_S^{\text{non-comm}}(t) = \int d\phi \, p(\phi,t) \, U_\phi(t)\rho_S(0)U_\phi^\dagger(t),
    \end{equation}
    where $U_\phi(t)$ are time-dependent unitary operators with phase-dependent rescaling factors, and $p(\phi,t)$ is a time-dependent probability distribution arising from the meter's Rabi oscillations. This demonstrates that the non-commuting case cannot be reduced to a simple sum over two branches—it requires integration over a continuous family of evolution operators, weighted by the meter's dynamical phase distribution.
    
    This analytical result matches the minima observed in the numerical simulations in the main text, confirming that the loss of speedup is due to the "washing out" of the coupling term by the transverse meter field, fundamentally altering the quantum channel beyond the two-branch description.
